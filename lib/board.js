    ...
    serialport.list(function(err, result) {
      ...
      var ports = result.filter(function(val) {
        ...
        if (Serial.used.includes(val.comName)) {
          available = false;
          j5("Port is not available because it is already used", Serial.used)
        }
  ...
  this.once("ready", function() {
    ...
    ["close", "disconnect", "error", "string"].forEach(function(type) {
      this.io.on(type, function(data) {
        j5("disconnect", this.io.port, this.io)
        Serial.used = Serial.used.filter(u => u !== this.io.port)
        this.emit(type, data);
      }.bind(this));
    }, this);
  }.bind(this));
